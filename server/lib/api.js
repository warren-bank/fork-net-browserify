...
module.exports = function (options, connectionListener) {
  ...
  if (options.allowOrigin) {
    ...
    if (allowOrigin && (typeof allowOrigin !== 'string') && (allowOrigin !== true)) {
      allowOrigin = false;
    }
    ...
    if (allowOrigin) {
      ...
      app.use(function (req, res, next) {
        ...
        res.header('Access-Control-Allow-Origin', (allowOrigin === true) ? req.headers.origin : allowOrigin);
  ...
  app.post(urlRoot + '/connect', jsonParser, function (req, res) {
    ...
    if (connectionListener) {
      connectionListener(socket, req.ip)
    }
  });
